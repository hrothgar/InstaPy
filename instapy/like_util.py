...
def check_link(browser, link, dont_like, ignore_if_contains, ignore_users,
  ...
  """Find the number of followers the user has"""
...
def like_image(browser):
  ...
  like_elem = browser.find_elements_by_xpath("//a[@role = 'button']/span")
  # browser.execute_script("var q=document.getElementsByClassName('" + like_elem[0].get_attribute("class") + "')[0];if(q.innerHTML=='Like'){q.click();}")

  if(like_elem[0].text == "Like"):
    browser.execute_script("document.getElementsByClassName('" + like_elem[0].get_attribute("class") + "')[0].click()")
    # print('--> Image liked!')
    return True

  else:
    # print('--> Already liked or invalid like element')
    return False
